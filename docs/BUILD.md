...
### Build the host-like MPU RPM (CLI + SOME/IP, stub cloud path)

This target packages the same runtime model used in local two-process
development (`platforms/posix-mpu`): interactive CLI, SOME/IP client,
in-process signal bus, and stub cloud transport.

```bash
# Native architecture
packaging/build-rpm.sh mpu-hostlike

# Cross-build for aarch64
packaging/build-rpm.sh mpu-hostlike aarch64
```

The script also creates a local DNF repository at
`build/rpm-repo/mpu-hostlike/`, so it can be consumed directly by
...
## Build an AutoSD image with host-like MPU behavior (AIB)

Use `packaging/aib/body-ecu-mpu-hostlike.aib.yml` to compose an image
that installs `body-ecu-mpu-hostlike` from the local repo generated by
`packaging/build-rpm.sh`.

```bash
# 1) Build the package and local repo
packaging/build-rpm.sh mpu-hostlike aarch64

# 2) Build an image with Automotive Image Builder
cd /Users/vzein/Projects/autosd-quickstart/sample-images
aib build --target qemu --arch aarch64 \
  /Users/vzein/Projects/spikes/body-ecu/packaging/aib/body-ecu-mpu-hostlike.aib.yml \
  localhost/body-ecu-mpu-hostlike:latest \
  body-ecu-mpu-hostlike.aarch64.qcow2
```

In the VM, run the MPU process the same way as on host:

```bash
body-ecu-mpu-hostlike 192.168.100.10
```
